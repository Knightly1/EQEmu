		...
		for(; check_cur != check_end; check_cur++) {
			...
			for(; cur2 != end2; cur2++) {
				...
				for(; cur3 != end3; cur3++) {
					...
					p1.x = git->x; p1.y = git->y; p1.z = git->z;
					p2.x = (*cur3)->x; p2.y = (*cur3)->y; p2.z = (*cur3)->z;
...
//	big->nodes.reserve(big->nodes.size() + db_spawns.size());
	...
	for(; cur3 != end3; cur3++) {
		...
		for(; cur != end; cur++) {
			...
			//check LOS
//			p2.x = (*cur)->x; p2.y = (*cur)->y; p2.z = (*cur)->z;
			//elevate a little, to about eye height
//			p2.z += 6.0f;
...
//int r = 0;
	for(; cur != end; cur++, curn++) {
//printf("Filling node %d/%d n=0x%x, lb=0x%x, curl=0x%x/0x%x\n", r++, index, *cur, linkBlock, curl, &linkBlock[eoffset]);
		n = *cur;
		if(n == NULL) {
