#ifndef EQE_PATCH_051105_H
#define EQE_PATCH_051105_H
...
/*0640*/	int32	race[10];			// Characters Race
/*0680*/	int8	face[10];			// Characters Face Type
/*0690*/	int8	haircolor[10];			// Characters Hair Color
/*0700*/	int8	beard[10];			// Characters Beard Type
/*0710*/	int8	eyecolor1[10];			// Characters Eye Color
/*0720*/	int8	gender[10];			// Characters Gender
/*0730*/	int8	eyecolor2[10];			// Characters Eye 2 Color
/*0740*/	Color_Struct	cs_colors[10][9];	// Characters Equipment Colors
/*1100*/	int8	unknownx[2];
/*1102*/	int8	level[10];			// Characters Levels
/*1112*/	int32	equip[10][9];			// 0=helm, 1=chest, 2=arm, 3=bracer, 4=hand, 5=leg, 6=boot, 7=melee1, 8=melee2  (Might not be)
/*1472*/	int8	beardcolor[10];			// Characters beard Color
/*1482*/	int8	unknown1482[2];
/*1484*/	int32	zone[10];			// Characters Current Zone
/*1524*/	int8	gohome[10];			// 1=Go Home available, 0=not
/*1534*/	int8	hair[10];			// Characters hair style
/*1544*/	int32	deity[10];			// Characters Deity
/*1584*/	int32	melee[2][10];			// Characters primary and secondary IDFile number
/*1664*/	int8	class_[10];			// Characters Classes
...
*/
/*
	Missing fields found using mobs in:
	PoTranquility
	Bazaar
	PoKnowledge
*/
struct Spawn_Struct
{
//there is supposed to be a flag in here for is_boat;

/*0000*/ uint8  is_pet;
/*0001*/ uint8	face;			//not verified
/*0002*/ uint8	unknown0002;
/*0003*/ uint8	is_npc;
/*0004*/ uint8	unknown0004;
/*0005*/ uint8	aa_title;
/*0006*/ uint8	bodytype;
/*0007*/ uint8	unknown0007[3];
/*0010*/ uint8	hairstyle;			//not verified
/*0011*/ uint8	NPC;
/*0012*/ float    size;           // Model size
/*0016*/ char     title[32];      // Player's title
/*0048*/ uint32 guild_id;        // Current guild
/*0052*/ uint8  invis;          // Invis (0=not, 1=invis)
/*0053*/ float    walkspeed;      // Speed when walking
/*0057*/ int16  deity;          // Player's Deity
/*0059*/ uint8  unknown0059[2]; // ***Placeholder
/*0061*/ uint8  gender;         // 0=male, 1=female, 2=other
/*0062*/ uint16 spawnId;        // Spawn Id
/*0064*/ uint8  unknown0064[6]; // ***Placeholder
/*0070*/ uint8 guild_rank;		//0 = normal, 1 = officer, 2 = guildleader
/*0071*/ uint8 unknown71[7];
/*0078*/ sint32 deltaX:13,
		   y:19;
/*0082*/ sint32 deltaY:13,
		   z:19;
/*0086*/ int32	heading:12, // Change in heading 
		deltaHeading:10, // Spawn heading 
		animation:10; // animation id 
/*0090*/ sint32 x:19,
		   deltaZ:13;
/*0094*/ float    unknown90;		//seems to range from 3-12ish
/*0098*/ char     suffix[32];     // Player's suffix (of Veeshan, etc.)
/*0130*/ uint8  class_;         // Player's class
/*0131*/ uint8  eyecolor1;			//not verified
/*0132*/ uint8  unknown0132[5]; // ***Placeholder
/*0137*/ uint8  light;       // Spawns' light source
/*0138*/ float    runspeed;       // Speed when running
/*0142*/ uint8  unknown0142[5];
/*0147*/ uint8	beard;
/*0148*/ sint8	cur_hp;
/*0149*/ uint8	findable;


//more wrongness to make it compile, these need a real home,, taking place of this unknown:
/*0150*/ uint8	unknown149[24];
/*//int8	aa_title;		//this prolly dosent exist anymore
int8	invis2;
int8	pvp;
int8	linkdead;

//int8	max_hp;		//I dont see this in the packet, I hope it is gone
int8	gm;
*/
/*0174*/ uint8	equip_chest2; //chest texture (usually 0xFF in live packets)
		// Not sure why there are 2 of them, but it effects chest texture!
		// drogmor: 0=white, 1=black, 2=green, 3=red
		// horse: 0=brown, 1=white, 2=black, 3=tan
/*0175*/ uint8  level;          // Player's Level
/*0176*/ uint8  anon;           // 0=normal, 1=anon, 2=roleplay
/*0177*/ uint8  beardcolor;		//not verified (might swap with haircolor)
/*0178*/ uint8  unknown0178;
/*0179*/ uint32 equipment[9];  // Array elements correspond to struct equipment above
/*0215*/ Color_Struct dye_rgb[9]; // Array elements correspond to struct equipment_colors above
/*0251*/ char     name[64];        // Player's Name
/*0315*/ uint16 petOwnerId;
/*0317*/ uint8  unknown0317[2];  //
/*0319*/ uint8  afk;
/*0320*/ uint8	haircolor;		//not verified (might swap with beardcolor)
/*0321*/ uint8	helm;
/*0322*/ char     lastName[32];    // Players last name
/*0354*/ uint8  unknown0354[5]; //***Placeholder
/*0359*/ uint8	set_to_0xFF[8];
/*0367*/ uint8	eyecolor2;
/*0368*/ uint8  lfg;             // 0=off, 1=lfg on
/*0369*/ uint8  unknown0369[8]; //***Placeholder
/*0378*/ int32	race;
/*0382*/ uint8  unknown0381[2];
...
#define MAX_PLAYER_BANDOLIER 4
#define MAX_PLAYER_BANDOLIER_ITEMS 4
...
#define MAX_LEADERSHIP_AA_ARRAY 32
...
#define MAX_PP_MEMSPELL		9
#define MAX_PP_SKILL		75
#define MAX_PP_AA_ARRAY		240
...
/*3812*/	uint8				unknown3812[24];        // @bp unknown skills?
...
/*064*/	uint32	zoneID;
/*068*/	int8	zone_reason;	//0x0A == death, I think
/*069*/ int8	unknown69[3];
...
{
/*00*/	uint16	spawn_id;
/*02*/	uint16	sequence;	//increments one each packet
/*04*/	uint16 heading:12,
		padding0004:4;
/*06*/	uint8 unknown0006[2];
/*08*/	float delta_y;            // Change in y
/*12*/	float x_pos;                 // x coord
/*16*/	float delta_x;            // Change in x
/*20*/	sint32	delta_heading:10,  // Change in heading
         		animation:10,
         		unknown20:12;
/*24*/ float z_pos;                 // z coord
/*28*/ float y_pos;                 // y coord
/*32*/ float delta_z;            // Change in z
};
...
struct SetServerFilter_Struct {
	...
	/*0084*/ int32	unknown84;
	/*0088*/ int32	unknown88;
	/*0092*/ int32	unknown92;
	/*0096*/ int32	unknown96;
	/*0100*/
};
...
struct GuildMember{
	...
	int32	rank;
	int32	timelaston;
	int32	guild_tribute_flag;
	int32	guild_tribute_donated;
	int32	last_tribute_donation_time;
...
**********************************************************************************************************/	
	...
	//Declare our packet file writer
