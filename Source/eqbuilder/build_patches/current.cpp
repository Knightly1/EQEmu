...
void BuildFileWriter_Current::WritePacket(uint16 eq_op, uint32 packlen, unsigned char *packet) {
	if(out == NULL)
		return;
...
printf("Spawn(%u): '%s' (%.4f,%.4f,%.4f) %.4f\n", us.spawn_id, us.name, us.x, us.y, us.z, us.heading);
...
printf("Add(%u): '%s' (%.4f,%.4f,%.4f) %.4f\n", us.spawn_id, us.name, us.x, us.y, us.z, us.heading);
	...
	case OP_MobUpdate: {
		...
		//this seems wrong, this should only be 12 bits...
		us.heading = EQ19toFloat(them->heading);
printf("WP(%u): (%.4f,%.4f,%.4f) %.4f\n", us.spawn_id, us.x, us.y, us.z, us.heading);
		
		_WriteBlock(PF_OP_MobLocation, &us, sizeof(us));
		break;
	}
	case OP_ClientUpdate: {
		if(packlen == sizeof(PlayerPositionUpdateClient_Struct)) {
			//this is a client => server movement packet.
			return;
		}
		if(packlen != sizeof(PlayerPositionUpdateServer_Struct)) {
			printf("Error: invalid size of OP_ClientUpdate packet (%d != %d)\n", packlen, sizeof(PlayerPositionUpdateServer_Struct));
			return;
		}
	...
	case OP_ItemPacket: {
		...
		if (!ItemParse(them->SerializedItem,packlen-sizeof(ItemPacket_Struct),
